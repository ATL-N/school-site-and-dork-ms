    ...
    sqlQuery = `
      WITH class_data AS (
        ...
        WHERE
          ...
          ${
            query
              ? "AND (c.class_name ILIKE $1 OR c.class_level ILIKE $1 OR c.room_name ILIKE $1)"
              : ""
          }
        GROUP BY
          c.class_id, s.last_name, s.first_name, s.middle_name
        ORDER BY c.class_name ASC, c.class_level ASC  -- Added sorting here
      ),
      overall_stats AS (
        SELECT
      ...
      SELECT
        ...
        json_agg(
          ...
          ORDER BY cd.class_name ASC, cd.class_level ASC  -- Explicit sorting in JSON aggregation
        ) AS classes
      FROM
        class_data cd
    `;
